...
Consider buying concert tickets online. There's one seat left for The Weeknd, and Alice and Bob both want it. They each hit "Buy Now" in the same instant. The obvious way to handle a purchase is to read the current seat count, check that it's above zero, then decrement it and sell the ticket.
...
The race exists because the read and write aren't atomic. Reading the count and writing the new value back are two separate steps, and in between them the world can change. Bob slips into that gap, reads a seat count Alice is about to invalidate, and acts on a number that's already stale. The window is tiny, microseconds in memory and milliseconds over a network, but it's all it takes for both requests to decide they've won the same seat.
...
[Flash Sale](https://www.hellointerview.com/learn/system-design/problem-breakdowns/flash-sale#dealing-with-contention)

## The Solution

This race condition is a **lost update**, the classic failure of a **read-modify-write** cycle. Two requests read the same value, both act on it, and both write back, so one silently overwrites the other.

There are several ways to make the read-modify-write safe. Each relies on the same compare-and-set technique, a read-modify-write where you make the write conditional on the value not having changed. We'll write the examples in SQL, but the same moves live in any serious datastore, and we'll add coordination only as each approach hits its limit. Start with conditional writes.
...
The database can enforce "decrement only if a seat is left" in one statement:
...
Conveniently, this is already safe under concurrency. No matter how many people try to grab the last ticket at once, only one will succeed. The reason is that the database won't let two updates change the same row at the same time. It makes them take turns, and the one that has to wait doesn't get to act on the value it first read. Say Alice and Bob run this at the same instant. Alice's update goes first and takes the count from 1 to 0. Bob's waits for her to finish, re-checks available\_seats > 0 against the row she left behind, finds it false, and matches zero rows. One seat, one sale. The check and the decrement are a single atomic step, so there's no gap for Bob to slip into.

The exact failure behavior depends on your database's isolation level, which we get to later. At the common defaults it re-checks the condition and matches zero rows, exactly as above. At stricter levels the database instead makes the loser abort with a serialization error that your app retries. Either way it's one seat, one sale. Only the failure shape changes.
...
Combining the two writes does introduce another issue, though. When an UPDATE matches zero rows, meaning the WHERE clause found nothing to change, that's not an error. The statement succeeds, it just did nothing, so it won't fail the transaction or force a rollback. Run the INSERT unconditionally and Bob's failed decrement still creates a ticket for a seat he never got. The fix is to make the insert depend on whether the update actually changed anything.
...
The guard must protect the resource being contended. A counter can answer "is there a ticket left." Only the ticket row can answer "is this ticket left."
...
Say a group of four wants to sit together. Now picking the seats isn't a single decision the database can make for you. Your app has to read the seat map, find four open seats in a row, and only then claim them. Because seat selection happens in application code between the read and the write, another transaction can modify those seats in between. Finding a contiguous block isn't a predicate you can drop into a WHERE clause, so the read and the write can't collapse into one statement. Two groups both read the same map, both land on A15 through A18, and both move to claim it, and whichever writes second takes seats the other had already picked. We're right back to the lost update.

This is where you need an explicit row lock. **Pessimistic locking** prevents conflicts by acquiring locks upfront. The name comes from being "pessimistic" about conflicts, assuming they will happen and blocking them before they can.
...
A conditional UPDATE doesn't work here. With the counter, the check was a predicate the database could evaluate as it wrote, so a single statement did the whole job. With a block of seats, the decision runs in your application between the read and the write, and the database can't redo it inside a WHERE clause. So you lock the rows you read, decide while they're held still, and write before letting go. The same shape covers any read-decide-write where the decision isn't a predicate. Checking a balance against a daily limit before moving money, or reading stock across several items before claiming a bundle, both need the lock for the same reason. And if your logic ever simplifies back down to something a single WHERE clause can express, drop the lock and go back to the conditional UPDATE. Don't pay for a lock you don't need.
...
The lock does its job in protecting resources from contention, but it's expensive. There are two common mistakes that are worth looking out for.
...
That's a distributed lock. It's the same locking idea from earlier, just held as a lease with its own lifetime instead of a transaction-scoped lock, and you'd use it any time exclusive access has to span multiple steps, a call to another service, or a wait. You have a few options for where that lease lives, and each trades off speed, infrastructure, and how gracefully it handles failure:
...
Whenever possible, keep the contended resource in one authoritative store. Every technique here depends on it. If you need strong consistency under high contention, do everything you can to keep the relevant data in a single database first. Nine times out of ten that's possible. Two situations break the single-home assumption, and both live outside this pattern. If one *operation* must atomically span multiple services or shards (like a transfer between two sharded accounts), that's a distributed transaction, covered in [Multi-step Processes](https://www.hellointerview.com/learn/system-design/patterns/multi-step-processes). And if the same *record* is writable in multiple places at once (multi-leader or leaderless replication), there's no single home to defend, and you're into conflict resolution like last-write-wins, vector clocks, or CRDTs, which is a replication topic beyond our scope here.
...
## [In the Wild](https://www.hellointerview.com/learn/system-design/in-the-wild?pattern=dealing-with-contention)
...
* [![Figma](https://www.hellointerview.com/_next/static/media/figma.0ni35n1gperui.svg?dpl=496e216d3ce87e7f3171c9d940be7a9f2d93afb8)How Figma Built Multiplayer Editing on Simplified CRDTs](https://www.hellointerview.com/learn/system-design/in-the-wild/figma-multiplayer)
...
[Next: Multi-step Processes](https://www.hellointerview.com/learn/system-design/patterns/multi-step-processes)

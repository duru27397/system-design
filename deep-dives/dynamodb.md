...
How to use DynamoDB in system design interviews: partition keys, secondary indexes, consistency modes, transactions, and DAX caching, plus when to choose it.
...
**Tables** - Serve as the top-level data structure in DynamoDB, each defined by a mandatory primary key that identifies its items. Tables support secondary indexes, enabling queries on non-primary key attributes for more versatile data retrieval.
...
![Create Table](https://www.hellointerview.com/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fdynamo-create-table.02ifgxoe-cmgv.png&w=3840&q=75&dpl=496e216d3ce87e7f3171c9d940be7a9f2d93afb8)
...
1. **Partition Key** - A single attribute that, along with the sort key (if present), identifies each item in the table. DynamoDB uses the partition key's value to determine the physical location of the item within the database. This value is hashed to determine the partition where the item is stored.
...
[Next: PostgreSQL](https://www.hellointerview.com/learn/system-design/deep-dives/postgres)
